...
from src.catalog import ordered, validate
...
def check(path, root=ROOT):
    ...
    def destination_page(dest):
        if isinstance(dest, str):
            target = named_destinations.get(dest)
    ...
    book = meta.get('book_choices')
    if book:
        validate(book, cat)
        ids = [e['id'] for e in meta['essays']]
        selected = [r['id'] for section in book['sections'] for r in ordered(book, cat, section)
                    if book['essays'][r['id']]['included']]
        expected_ids = selected if meta.get('kind') == 'full' else [k for k in selected if k in ids]
        if ids != expected_ids:
            issues.append('Exported selection/order differs from saved book choices.')
        if set(meta['intentionally_excluded']) != {k for k, v in book['essays'].items() if not v['included']}:
            issues.append('Excluded entries differ from saved book choices.')
    expected_outline = [('Cover', 0), ('Title page', 1), ('Contents', 2)]
    previous = None
    for e in meta['essays']:
        if e['section'] != previous:
            expected_outline.append((e['section'], e['start_page'] - 1))
            previous = e['section']
        expected_outline.append((e['title'], e['start_page'] - 1))
    if outline_entries != expected_outline:
        issues.append('Bookmark order or section destinations differ from the book.')
    contents_targets = set()
    for e in meta['essays']:
    ...
    with pdfium.PdfDocument(path) as doc:
        for i, p in enumerate(reader.pages):
            ...
            for ref in p.get('/Annots', []):
                ...
                if dest is not None:
                    internal += 1
                    if i < first_reading and destination_page(dest) != meta.get('contents_start_page', 3) - 1:
                        contents_targets.add(destination_page(dest))
            ...
            body = tp.get_text_bounded(left=g['text_left']-3, right=g['text_left']+g['text_width']+3,
            ...
            if i >= first_reading:
                ...
                if len(body.strip()) < 180:
                    sparse.append(i+1)
                # PDFium groups text on a line into rectangles. Testing their
                # union bounds avoids millions of per-character API calls.
                for ri in range(tp.count_rects()):
                    ...
                    if y_bottom < g['text_top'] - 1 or y_top > s['page_height'] - s['margin_bottom'] - s['footer_height']:
                        continue
                    # Full-book italic f/fi ink overhangs up to 2.89 pt;
                    # it remains inside the 14 pt gap, outside writing space.
                    if left < g['text_left'] - 3 or right > g['text_left'] + g['text_width'] + 3 or y_bottom > g['text_top'] + g['text_height'] + 1:
            ...
            tp.close(); page.close()
            if (i + 1) % 100 == 0:
                print(f'Checked PDF pages {i + 1}/{len(reader.pages)}; issues so far: {len(issues)}.', flush=True)
                path.with_suffix('.audit-progress.json').write_text(json.dumps(dict(pages_checked=i+1, issues=issues, sparse=sparse)), encoding='utf-8')
    if contents_targets != {e['start_page'] - 1 for e in meta['essays']}:
        issues.append('Contents does not link to every selected essay opening.')
    source_records = {r['id']: r for r in cat['essays']}
    preservation = []
    for e in meta['essays']:
        opening = normal(texts[e['start_page']-1])
        if normal(e['title']) not in opening[:len(normal(e['section']))+len(normal(e['title']))+20]:
            issues.append('Essay heading missing at recorded opening: ' + e['title'])
        content, evidence = prepare(source_records[e['id']], root, s.get('note_returns', True), s.get('include_images', True))
