...
def validate(s, root=ROOT):
    ...
    if type(s['include_images']) is not bool:
        raise ValueError('include_images must be true or false.')
    if type(s['contents_columns']) is not int or s['contents_columns'] not in (1, 2, 3):
        raise ValueError('Contents columns must be 1, 2 or 3.')
    if not 8 <= s['contents_size'] <= 18:
        raise ValueError('Contents text must be 8 to 18 pt.')
